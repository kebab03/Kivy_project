...
class Root(FloatLayout):
    ...
    def uploader(self,plec):
        ...
        path_local=plec
        infs=len(path_local)
        print('lpath_local prima)',infs)
        if len(path_local)>2:
            infs=len(path_local)
            print('lpath_local)',infs)
            ...
            #try:        
            print('len(path_local):',len(path_local))
            # path_local=r'C:\Users\MINE\Pictures\leeon data\screenshoot\2021-05-11 12_18_43-Greenshot.png'
            if storage.child(path_on_cloud).put(path_local):
                print('storage::',storage)
            #self.text_output.text = 'uploaded' 
            else :
                print('sono mancat directory?')          
        #except:
            #pass            
    ...
    def upload(self):
        ...
        if plec.find('.jpg')>0:
            ...
            print('png ind upload;;::',plec.find('.png'))
            #self.ids.image.source = filename[0]
            self.text_input.text = 'uploading png'
            self.uploader(plec)
        elif plec.find('.txt')>0:
    ...
    def save(self, path, filename):
        ...
        self.dismiss_popup()
    def clear(self):
        self.text_input.text = 'cancellto tutto'
